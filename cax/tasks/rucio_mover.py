...
class RucioBase(Task):
    ...
    def list_rse_usage(self, rse_remote):
        ...
        for i in msg_std:
            if i.find("[RSE does not exist") >= 0:
                ...
                rse_usage_summary['rse_usage_used'] = i.split(":")[
                    1].replace(" ", "")
            elif i.find("rse:") >= 0:
                rse_usage_summary['rse_usage_rse'] = i.split(":")[
                    1].replace(" ", "")
                ...
                rse_usage_summary['rse_usage_source'] = i.split(":")[
                    1].replace(" ", "")
    ...
    def list_file_rules(self, location):
        ...
        files, file_info = self.list_files(
        ...
        for i_file in files:
            ...
            for i in msg_std:
                if i.find(new_location) >= 0:
                    ...
                    if len(line) > 6:
                        single_rule['rule_expired'] = "{date}_{time}".format(
                            date=line[6], time=line[7])
                    else:
    ...
    def list_rules(self, location, rse_remote):
        ...
        for i in msg_std:
            if i.find(rse_remote) >= 0 and i.find(config.get_config(self.remote_host)["rucio_account"]) >= 0:
                ...
                if len(i1) >= 7:
                    rule_summary['expires'] = "{date}_{time}".format(
                        date=i1[6], time=i1[7])
                elif len(i1) < 7:
    ...
    def download(self, location, rse_remote, download_dir):
        ...
        for i in msg_std:
            if i.find("No such file or directory:") >= 0:
                logging.info(
                    "The requested download directory %s does not exists or something else is bad [ERROR]", download_dir)
            elif i.find("has no replicas available on disk endpoints and cannot be downloaded. Please ask for a replication") >= 0:
                logging.info(
                    "There is no replica of %s at RSE %s [ERROR]", location, rse_remote)
            elif i.find("WARNING [Provided RSE expression is considered invalid.") >= 0:
                logging.info(
                    "The download was done without specifiying the RSE before! [WARNING]")
    ...
    def delete_rule(self, location, rse_remote):
        ...
        if len(msg_std) == 0:
            ...
            logging.info("Request to delete rule %s is sent",
            ...
            for i_msg in msg_std:
                if i_msg.find("ERROR [A RSE expression must be specified if you do not provide a rule_id but a DID]") >= 0:
                    logging.info(
                        "Ups... Something went wrong with your rule ID")
        ...
        if i_rule_id != "n/a" and int(lifetime) > 0:
            ...
            for i in msg_std:
                if i.find("Updated Rule") >= 0:
                    logging.info(
                        "Update rule sucessful from valid to %s seconds unitl termination.", lifetime)
    ...
    def set_rule(self, location, rse_remote, lifetime="-2"):
        ...
        if rule_summary['status'].find("OK") >= 0 and i_rule_id != "n/a":
            ...
            if len(files) == 1:
                pathlists = self.get_file_locations_keep(
                    location.split(":")[0], files)
            else:
                pathlists = self.get_file_locations(
                    location.split(":")[0], files)
            ...
            i_path = super_string
            logging.info("Status of transfer %s to RSE %s: OK",
                         location, rse_remote)
            i_rule_status = "OK"
            ...
            if int(lifetime) > 0:
                ...
                logging.info(
                    "Update the rule for %s with setting to %s sec", rse_remote, lifetime)
            ...
            logging.info(
                "Status of transfer %s to RSE %s: REPLICATING", location, rse_remote)
            i_path = "n/a"
            i_rule_status = "REPLICATING"

        elif rule_summary['status'].find("STUCK") >= 0 and i_rule_id != "n/a":
            logging.info("Status of transfer %s to RSE %s: STUCK",
                         location, rse_remote)
            i_path = "n/a"
            i_rule_status = "STUCK"
            ...
            for i in msg_std:
                if i.find("ERROR [Data identifier not found.") >= 0:
                    logging.info(
                        "Error: Data identifier %s does not exists!", location)
                    i_rule_id = "n/a"
                    i_rse = rse_remote
                    i_path = "n/a"
                    i_rule_status = "n/a"
                    i_expired = "n/a"
                    i_rule_account = config.get_config(self.remote_host)[
                        "rucio_account"]
                    break
                elif i.find("Details: There is insufficient quota") >= 0:
                    logging.info(
                        "Error: There is insufficient quota on %s to fullfill the operation!", rse_remote)
                    i_rule_id = "n/a"
                    i_rse = rse_remote
                    i_path = "n/a"
                    ...
                    logging.info(
                        "Error: The creation of the replication rule failed at this time. Please try again later.")
                    i_rule_id = "n/a"
                    i_rse = rse_remote
                    i_path = "n/a"
                    i_rule_status = "Failed"
                    ...
                    logging.info(
                        "Rule already exists for location=%s and RSE=%s", location, rse_remote)
                    #rule_summary = self.list_rules( location, rse_remote)
                    ...
                    logging.info("A new rule is created for %s (%s)",
                                 rule_summary_new['rse'], rule_summary_new['rule_id'])
    ...
    def query_transfer_tags(self, location, ttype):
        ...
        transfer_tag_created_at = tag_created_at.date().strftime(
            "%Y%m%d")[2:] + "_" + tag_created_at.time().strftime("%H%M")
    ...
    def query_meta_tags(self, location, ttype):
        ...
        # just an option, not used in meta data set (see w)
        meta_tag_name = tags_all["name"]
        meta_tag_source_type = tags_all["source"]["type"]
        meta_tag_runnumber = tags_all["number"]
        meta_tag_sub_detector = tags_all["detector"]
        tag_created_at = tags_all["start"]
        meta_tag_created_at = tag_created_at.date().strftime(
            "%Y%m%d")[2:] + "_" + tag_created_at.time().strftime("%H%M")
        ...
        meta_tags[0].update(
            {"version": self.get_software_version(location, ttype)})
    ...
    def sanity_checks(self):
        ...
        # 0)Check if rucio is available on the host:
            ...
            logging.info(
                "Sanity check: Check for you Rucio installation at %s", config.get_hostname())
            return False

        # 1) Check if the specified rucio account exists
        elif self.check_rucio_account() == False:
            logging.info("Sanity check: The specified account %s does not exists in the current rucio list",
                         config.get_config(self.remote_host)["rucio_account"])
            logging.info(
                "Sanity check: Use \'rucio-admin account list\' manually!")
            return False
        ...
        #  to the Rucio catalogue.
            ...
            logging.info(
                "Sanity check: Attention: Check your json configuration file: RSE %s does not exists!", self.get_rucio_rse())
            return False
    ...
    def check_scope(self, scope_name):
        ...
        logging.debug(
            "The scope name %s exists: %s (message within check_scope)", scope_name, scope_excists)
        ...
        for i in msg_std:
            ...
            for irse in rse_list:
                if i.find(irse) >= 0 and i.find("|") == 0:
                    ...
                    file_location_sub['path'] = ii[5].split(
                        ":", 1)[1].replace(" ", "")
                    file_location_rse[irse] = file_location_sub
                    file_location[file_location_sub['name']
                                  ][irse] = file_location_sub
    ...
    def list_files(self, rscope, ifile):
        ...
        # A dictionary of with file names (key) and further information (value)
        file_list_name = []
    ...
    def doRucio(self, upload_string):
        ...
        # fastest way to remove '' from list
    ...
    def copyRucio(self, datum_original, datum_destination, option_type):
        ...
        raccount = config.get_config(datum_destination['host'])[
            "rucio_account"]

        # if data_type == "raw":
        logging.info("Start raw data upload to rucio catalogue")

        dataset_name, datapath, files = self.get_input_files(
        ...
        dt = datetime.datetime(int(str("20" + date_[0:2])), int(
            date_[2:4]), int(date_[4:6]), int(time_[0:2]), int(time_[2:4]))
        ...
        logging.info(
            "Rucio - Dataset name (depend on raw/processed upload type): %s", dataset_name)
        ...
        for i in msg_std:
            logging.info(
                "Rucio (add-container into scope %s): %s", rscope_basic, i)
        ...
        upload_folder_p = files[0].replace(files[0].split("/")[-1], "")
        data_identifiyer = "{scope}:{dname}".format(
            scope=rscope_upload, dname=dataset_name)
        upload_folder = self.RucioCommandLine(self.host,
        ...
        for i in msg_std:
            if i.find("ERROR [The requested service is not available at the moment.") >= 0:
                ...
                logging.info(
                    "ERROR: Your RUCIO_ACCOUNT environment variable not match with a registered identity to your account.")
                self.return_rucio['checksum'] = "n/a"
                self.return_rucio['location'] = "n/a"
                self.return_rucio['rse'] = []
                self.return_rucio['status'] = "RSEreupload"
        ...
        entrance_rse = config.get_config(datum_destination['host'])[
            "rucio_upload_rse"]
        ...
        for i_file in files:
            for key_filename, value in file_locations.items():
                if i_file.find(key_filename) >= 0 and entrance_rse in value:
                    ...
                    logging.info("checksum test: %s -> %s (local) and %s (rucio)",
                                 local_file, local_cksum, rucio_cksum)
        ...
        if cnt_cksum == len(files):
            ...
            self.return_rucio['status'] = "RSEreupload"
            logging.info(
                "Upload status: error - Files or checksums do not agree!")
            return
    ...
    def RucioCommandLine(self, host, method, filelist=None, metakey=None):
        ...
        general = RucioBashConfig.load_host_config(
            config.get_hostname(), "py2")
      ...
      """
        if filelist is not None and method == "upload-advanced":
            for ifile in filelist:
                upload_adv += "rucio upload --rse {rse} --scope {scope} " + str(
                    ifile) + " \n"
      ...
      """
        if metakey is not None and method == "set-metadata":
            ...
            for i in metakey_list:
                set_metadata += "rucio set-metadata --did {scope}:{dataset} --key " + str(
                    i[0]) + " --value " + str(i[1]) + "\n"
...
rucio add-dataset {scope}:{dataset}      
      ...
      """
        if filelist is not None and metakey == None:
            for ifile in filelist:
                attach += "rucio attach {up_scope}:{up_did} {scope}:" + \
                    ifile.split("/")[-1] + "\n"
...
rucio download --no-subdir {rse_dw} {dir} {scope}:{name}
    ...
    def get_files(self, args):
        ...
        if args.count(':') == 1:
            logging.warning(
                "{0} cannot be distinguished from scope:datasetname. Skipping it.".format(args))
        ...
        if os.path.isdir(args):
            ...
            if fnames:
                ...
                raise Exception(
                    "Directory ({directory}) has no files in it. Please add subdirectories individually.".format(directory=dname))
            else:
                raise Exception(
                    "Directory ({directory}) is empty.".format(directory=dname))

        elif os.path.isfile(args):
            files.append(args)
        else:
            logging.warning(
                '{0} is not a directory or file or does not exist'.format(args))
            return []
        return files
    ...
    def get_input_files(self, option_type, data_type):
        ...
        if config.RUCIO_UPLOAD is not None:
            ...
            if is_file == True and is_folder == False and f_exists == True:
                ...
                # get dataset name for dataset creation
                dataset_name = pfolder.split("/")[-1]
            ...
            logging.info(
                "Transfer information %s (not used at the moment)", transfer_tags)
    ...
    def each_run(self):
        ...
        if self.method == "SingleRun":
            ...
            for data_doc in self.run_doc['data']:
                ...
                status = data_doc['status']
                location = data_doc['location']
                logging.info("<%s> : Location: %s | Run name: %s | Run number: %s | RSE: %s", status,
                             location, self.run_doc['name'], self.run_doc['number'], ', '.join(rse_storage))
            ...
            rucio_conf = rucio_basic_conf_class.load_host_config(
                config.get_hostname(), "py2")
            ...
            for data_doc in self.run_doc['data']:
                ...
                list_dataset_rules = self.rucio.list_all_rules(
                    data_doc['location'])
                logging.info("Rucio DID %s:%s has:", scope, dname)
                for key, num in list_dataset_rules.items():
                    logging.info("RSE: %s -> Rule: %s (%s)", key,
                                 num['rule_id'], num['rule_expired'])

                logging.info("-----individual rules (if exists)-----")
                list_sfile_rules = self.rucio.list_file_rules(
                    data_doc['location'])
                if len(list_sfile_rules) > 0:
                    for key, value in list_sfile_rules.items():
                        logging.info("Host %s | Rucio DID: %s:%s -> Rule: %s (%s)",
                                     value['rule_host'], scope, key, value['rule_id'], value['rule_expired'])
            ...
            for data_doc in self.run_doc['data']:
                ...
                if len(list_sfile_rules) > 0:
                    for key, value in list_sfile_rules.items():
                        ...
                        self.rucio.update_rule_force(
                            location_single, value['rule_host'], 10)
...
class RucioPurge(Task):
    ...
    def each_run(self):
        ...
        for data_doc in self.run_doc['data']:
            ...
            if len(data_doc['rse']) >= 1 and \
               ...
               nb_copies_tape_b == True and checksum_agree == True:
                ...
                logging.info("   Dataset: %s | Run number: %s",
                             self.run_doc['name'],  self.run_doc['number'])
                logging.info(
                    "   --------------------------------------------------")
                logging.info(
                    "   Checksum test for xe1t-datamanager and tape: %s", str(checksum_agree))
                logging.info("   Rucio dataset %s is on tape: %s",
                             data_doc['location'], nb_copies_tape_b)
                logging.info("   Rucio dataset %s is on xe1t-datamanager: %s",
                             data_doc['location'], nb_copies_xe1tdatamanager_b)
                logging.info(
                    "   Rucio dataset %s has one ore more copies at:", data_doc['location'])
        ...
        for data_doc in self.run_doc['data']:
            ...
            if data_doc['host'] == "xe1t-datamanager" and data_doc['status'] == "transferred" and \
               ...
               and checksum_agree == True:
                ...
                location = data_doc['location']
                logging.info("   Dataset %s is set for deletion.",
                             self.run_doc['name'])
                logging.info("   Location on xe1t-datamanager: %s", location)
                logging.info(
                    "   Purge mode is activate manually: %s", self.purge)
        ...
        if check_for_delete == False:
            ...
            logging.info("   Copy on xe1t-datamanager: %s",
                         nb_copies_xe1tdatamanager_b)
...
class RucioConfig():
    ...
    def load_host_config(self, host_select, py_version=None):
        ...
        if py_version == None:
            logging.info(
                "Please be sure that you request a certain python version!")
            logging.info("Choose: py2 / py3")
            exit()
        elif py_version == "py2":
            python_config = "rucio_config_p2"
            logging.debug(
                "Chosen python environment loaded by bash script: %s", py_version)
        elif py_version == "py3":
        ...
        if os.path.isfile(python_file) == True:
            ...
            logging.debug(
                "The file %s exists! Use this for python2.x/3.x configuration", python_file)
            h_load = open(python_file, 'r')
        else:
            # The requested bash script does not exists - Take the pre-installed configuration
            logging.debug("The file %s does not exists! Use pre-configured python2.x/3.x bash scripts for host %s",
                          python_file, config.get_hostname())

            p_path = os.path.realpath(__file__).replace(
                "tasks/rucio_mover.py", "")
            # Create the filename of the standard installation
            python_file = self.bash_config(config.get_hostname(), py_version)

            if os.path.join(p_path, "host_config", python_file) == False:
                logging.info(
                    "Check the file %s for the pre-installed bash configuration:", python_file)
                logging.info("Path: %s", p_path)
                logging.info(
                    "Hint: Checkout rucio_mover.py, memberfunction: bash_config() --> Check hard coded standard names")
                exit()
            h_load = open(os.path.join(
                p_path, 'host_config', python_file), 'r')
    ...
    def each_run(self):
        ...
        for data_doc in self.run_doc['data']:
            ...
            if self.data_dir == None:
                logging.info(
                    "Exit rucio-download: No download destination selected!")
            ...
            if self.data_restore == False:
                ...
                logging.info(
                    "Download to pre-selected folder: %s", self.data_dir)
                ...
                logging.info(
                    "There are already %s data registered at host %s -> No download necessary", self.data_type, self.data_host)
                exit()
                ...
                if os.path.exists(self.data_dir) and self.data_overwrite == False:
                    logging.info("The path %s exists already on host %s",
                                 self.data_dir, self.data_host)
                    logging.info(
                        "Exit rucio-download to avoid data loss/overwrite")
                ...
                logging.info(
                    "Restore to host %s from rucio-catalogue", self.data_host)
            ...
            logging.info("Number of already local files: %s",
                         result['alreadylocal_files'])

            logging.info("Number of downloaded files: %s", result['dw_files'])
            logging.info("Number of failed downloaded files: %s",
                         result['dwfail_files'])
            ...
            if count_checksum == len(lf[0]) and count_checksum == len(download_file_list):
                ...
                if self.data_restore == True and self.data_host == config.get_hostname() and self.data_host not in list_hosts and self.database_entry_extern == False:
                    ...
                    logging.info(
                        "New entry for Xenon1T data base: %s", datum_new)
...
class RucioRule(Task):
    ...
    def rule_definition(self):
        ...
        for i_rse in destination_rse:
            if i_rse.find("random") >= 0:
                ...
                logging.info(
                    "(rule_definition) - A list of RSEs: {rse} is given.".format(rse=random_rse))
                logging.info(
                    "(rule_definition) - Pick one random element: {e}".format(e=i_random_rse))

                destination_rse.remove(key)
                destination_rse.append(i_random_rse)
                destination_livetime[i_random_rse] = destination_livetime.pop(
                    key)
    ...
    def magic(self, actual_run, rule_def, all_rse):
        ...
        actual_run_name_t = time.mktime(datetime.datetime.strptime(
            actual_run['actual_run_name'], "%y%m%d_%H%M").timetuple())
        rule_run_name_list_t = []
        rule_run_name_range_t = []
        if rule_def['run_name_list'] != "all":
            for ilist in rule_def['run_name_list']:
                rule_run_name_list_t.append(time.mktime(
                    datetime.datetime.strptime(ilist, "%y%m%d_%H%M").timetuple()))
            for ilist in rule_def['run_name_range']:
                i_list_beg = time.mktime(datetime.datetime.strptime(
                    ilist[0], "%y%m%d_%H%M").timetuple())
                i_list_end = time.mktime(datetime.datetime.strptime(
                    ilist[1], "%y%m%d_%H%M").timetuple())
        ...
        logging.info("Verfication only status: %s",
                     rule_def['verification_only'])

        if rule_def == 0:
            # Define what happens if no additional rule definitions loaded:
            # Only upload to the entrance point
            logging.info("No additional rule definition is made: Upload to entrance point %s",
                         actual_run["actual_run_rse_entrance"])
            ...
            if rule_def['verification_only'] == True:
                ...
                if actual_run_name_bool == False and actual_run_number_bool == False:
                    logging.info(
                        "Actual run number matches not resquested run numbers from rucio-rule file.")
                    transfer_list = ["empty"]
                    for i_rse in transfer_list:
                        transfer_lifetime[i_rse] = "-2"
                ...
                logging.info(
                    "No extended transfer list is created to set or update rules")
                transfer_list = all_rse
                for i_rse in transfer_list:
                    transfer_lifetime[i_rse] = "-2"
    ...
    def set_possible_rules(self, data_type, dbinfo):
        ...
        if dbinfo != None:
            ...
            logging.info(
                "Means: There was a previous upload by do_possible_transfers() executed")
            there = dbinfo
        ...
        if there != None:
            ...
            transfer_list = config.get_config(
                "rucio-catalogue")['rucio_upload_rse']
            ...
            if rule_def != 0:
                ...
                transfer_list, transfer_lifetime, self.delete_list = self.magic(
                    actual_run, rule_def, all_rse)
            else:
                logging.info("No seperated rucio-rule file is loaded!")
                logging.info("Validation of entrance point rule at %s", config.get_config(
                    "rucio-catalogue")['rucio_upload_rse'])
            ...
            if "empty" in transfer_list:
                logging.info("Actual run number/name %s/%s",
                             actual_run['actual_run_number'], actual_run['actual_run_name'])
                logging.info(
                    "does not match with requested run number/name from rucio-rule configuration file")
                logging.info("   --> SKIP")
                return 0

            if there['status'] != "transferred":
                logging.info(
                    "RunDB status: %s - No need to create a transfer rule", there['status'])
                return 0
            ...
            for i_rse in all_rse:
                ...
                if i_rse in transfer_list:
                    for in_rse in transfer_list:
                        logging.info(
                            "Create or update a rule for RSE %s with a lifetime of %s", in_rse, transfer_lifetime[in_rse])
                        rule_result = self.rucio.set_rule(
                            rucio_location, i_rse, transfer_lifetime[i_rse])
                else:
                    logging.info(
                        "Just check RSE location %s (modus: %s)", i_rse, "-2")
                    rule_result = self.rucio.set_rule(
                        rucio_location, i_rse, "-2")

                logging.info(" * Status rule/transfer: %s",
                ...
                logging.info(" * Rule expires: %s",
                             rule_result['rule_expired'])
                logging.info(" * Rule account: %s",
                             rule_result['rule_account'])
                ...
                if rule_result['rule_status'] == "OK":
                    ...
                    rule_valid_to = "{rse}:{date}".format(
                        rse=i_rse, date=rule_result['rule_expired'])
            ...
            logging.info(
                "There is no runDB information for rucio-catalogue available: SKIP")
    ...
    def del_possible_rules(self, data_type, dbinfo):
        ...
        if all(x in actual_rse for x in self.delete_list) == False:
            ...
            for i in self.delete_list:
                logging.info(" * %s", i)
            logging.info(
                "do not match with the registered RSEs in the database")
            return 0
        else:
            remaining_rse = [x for x in actual_rse if str(
                x) not in self.delete_list]
        ...
        if there != None:
            ...
            for i_rse in self.delete_list:
                logging.info("Delete file %s from RSE %s",
                             there['location'], i_rse)
