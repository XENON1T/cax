    ...
    while True:  # yeah yeah
        ...
        for doc in docs:
            ...
            if 'cax_%s_v%s' % (job_name, pax.__version__) in qsub.get_queue():
                logging.debug('Skip: cax_%s_v%s job exists' %
...
def massiveruciax():
    # Command line arguments setup
    parser = argparse.ArgumentParser(
        description="Submit ruciax tasks to batch queue.")
    ...
    if args.from_run is None and args.to_run is None and args.last_days is None:
        pass
    elif (args.from_run != None and args.to_run == None) or (args.from_run == None and args.to_run != None):
        logging.info("Select (tpc) runs between %s and %s",
        ...
        logging.info("--> Ruciax exits here")
        exit()
    elif args.from_run != None and args.to_run != None and args.from_run > 0 and args.to_run > 0 and args.from_run < args.to_run:
        logging.info("Start (tpc) runs between %s and %s",
                     args.from_run, args.to_run)
        run_window = True
    ...
    if args.config_rule:
        ...
        verfication_only = json.loads(open(abs_config_rule, 'r').read())[
            0]['verification_only']
    else:
        verfication_only = False
        rucio_rule = ""
    ...
    while True:  # yeah yeah
        ...
        if run_lastdays == True:
            ...
            if t1 - t0 < (int(args.last_days) * dt):
                logging.info(
                    "Run ruciax up/downloads only on latest %s days", args.last_days)
                # See if there is something to do
                query['start'] = {'$gt': t0}
...
""".format(job=job)
            ...
            logging.info("+--------------------------->>>")
            logging.info(
                "| Summary: massive-ruciax for run/name: %s/%s", doc['number'], doc['name'])
            logging.info("| Configuration script: %s", runlogfile)
            logging.info("| Rucio-rule script: %s", abs_config_rule)
            logging.info("| Run time of ruciax: %s min %s",
                         str(dd[0]), str(dd[1]))
            logging.info(
                "+------------------------------------------------->>>")
        if run_once:
...
def massive_tsmclient():
    ...
    if args.run != None and args.name != None:
        logging.info(
            "Input to massive-tsm is --run {r} and --name {n} at the same time! <- Forbidden".format(r=args.run, n=args.name))
        exit()
    ...
    while True:  # yeah yeah
        ...
        if run_lastdays == True:
            ...
            if t1 - t0 < (int(args.last_days) * dt):
                logging.info(
                    "Run massive-tsm for up-/downloads only on latest %s days", args.last_days)
                # See if there is something to do
                query['start'] = {'$gt': t0}
...
def cax_tape_log_file():
    ...
    parser = argparse.ArgumentParser(
        description="This program helps you to watch the tape backup")
    ...
    if args.monitor == "logfile":
            ...
            for doc in docs:
                ...
                for idoc in doc['data']:
                    if idoc['host'] == "tsm-server" and idoc['location'] != "n/a" and idoc['location'].find("tsm/") >= 0:
                        nb_tsm_hosts += 1
                        logging.info("Dataset: %s at host %s",
                                     idoc['location'], idoc['host'])
                        fsize = tsm_mover.TSMDatabaseCheck(
                        ).get_info(idoc['location'])
                        total_uploaded_datasets += 1
                        logging.info(
                            "Total file size of the individual dataset: %s mb", fsize)
        ...
        logging.info("Total amount of uploaded data: %s mb",
                     total_uploaded_amount)
        logging.info("Total amount of uploaded data sets: %s",
                     total_uploaded_datasets)

    elif args.monitor == "checkstatus":
        if args.status == None:
            logging.info(
                "ATTENTION: Specify status by --status [transferred/transferring/error]")
            return 0
...
def ruciax_status():
    parser = argparse.ArgumentParser(
        description="Allow to check the run database for rucio entries")
...
def ruciax_download():
    ...
    if args.name is None and args.run is None and args.list_file is None:
    ...
    if args.list_file != None and args.name == None and args.run == None:
        ...
        if os.path.exists(args.list_file) == False:
            logging.info(
                "The requested file %s does not exists -> exit", args.list_file)
            exit()
        ...
        for i_line in lines:
            ...
            rucio_mover.RucioDownload(args.data_rse, args.data_dir, args.data_type,
                                      args.restore, args.location, args.overwrite).go(number_name)
...
def ruciax_locator():
    ...
    rucio_mover.RucioLocator(args.rse, args.copies,
                             args.method, args.status).go(number_name)
