    ...
    def write_outer_dag(self, outer_dag):
        ...
        with open(outer_dag, "wt") as outer_dag_file:
            ...
            for run in self.runlist:
                ...
                outer_dag_file.write("RETRY %s 3\n" % inner_dagname)
...
universe = vanilla
Error = {logdir}/$(pax_version)/$(name)/$(zip_name).log
Output  = {logdir}/$(pax_version)/$(name)/$(zip_name).log
Log     = {logdir}/$(pax_version)/$(name)/joblogs/$(zip_name).joblog
