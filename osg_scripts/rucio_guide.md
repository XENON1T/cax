...
<!-- ### Rucio Data Organization

Files can be grouped into datasets and datasets can be grouped into containers, and containers are grouped into containers. All three types of names refer to data so the term ‘data identifier set’ (DIS) is used to mean any set of file, dataset or container identifiers. A DID is just the name of a single file, dataset or container.

The scope string partitions the name space into several sub spaces. c
...
## Rucio Admin - ADMIN ONLY

NOTE: This is only for users with privileged accounts. Others can ignore.
